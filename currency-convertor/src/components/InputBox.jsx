...
const InputBox = ({label,amount,onAmountChange,onCurrencyChange,currencyOptions=[],selectCurrency="usd",amountDisabled=false,currencyDisabled=false,className=""}) => {
  ...
  return (
    <div className={`bg-white p-3 flex rounded-lg ${className}`}>
        ...
        <input id={amountInputId} type="number" className='w-full outline-none bg-transparent py-1.5' placeholder='Amount' disabled={amountDisabled} value={amount} onChange={(e)=> onAmountChange && onAmountChange(Number(e.target.value))}/></div>
